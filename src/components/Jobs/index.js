...
class Jobs extends Component {
  state = {
    ...
    profileData: {},
    jobsApiStatus: apiConstant.initial,
    jobsList: [],
    input: '',
    employmentType: '',
    minPackage: '',
  ...
  getProfile = async () => {
    ...
    if (response.ok) {
      ...
      this.setState({profileData, apiStatus: apiConstant.success})
    } else {
  ...
  getJobs = async () => {
    ...
    this.SetState({jobsApiStatus: apiConstant.inProgress})
    const jobs apiUrl = `https://apis.ccbp.in/jobs?employment_type=${employmentType}&minimum_package=${minPackage}&search=${input}`
    ...
    const response = await fetch(jobs apiUrl, options)
    ...
    if (response.ok === true) {
      ...
      this.setState({
        jobs apiStatus: apiConstant.success,
        jobsList: fetchedJobsData,
      })
    } else {
      this.setState({jobs apiStatus: apiConstant.failure})
    }
  }
  ...
  renderJobs = () => {
    const {jobs apiStatus, jobsList} = this.state
    switch (jobs apiStatus) {
      case apiConstant.inProgress:
        return this.renderLoadingView()
      case apiConstant.success:
